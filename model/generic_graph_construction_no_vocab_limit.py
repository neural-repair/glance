...
import os
from graph4nlp.pytorch.data.dataset import GraphData
...
class GenericGraphConstruction(DependencyBasedGraphConstruction):
    """
    ...
    def __init__(
        ...
        super(GenericGraphConstruction, self).__init__(
            vocab=vocab,
    ...
    def topology(
        ...
        opt = get_args()
        root_dir = opt["graph_construction_args"]["graph_construction_share"]["root_dir"],

        file_name = "{0}/merged-dataset/{1}-buggy.txt".format(root_dir[0], raw_text_data)
        print("loading correct file: {}".format(file_name))
        if not os.path.exists(file_name):
            empty_graph = GraphData()
            empty_graph.add_nodes(1)
            empty_graph.node_attributes[1]["token"] = 'dummy'
            print("buggy dot file:{} does not exist".format(file_name))
            return empty_graph

        #from line_to_graph_no_vocab_limit import json_cfg_and_dataflow_to_graph_token_based
        #return json_cfg_and_dataflow_to_graph_token_based(file_name)
        #from line_to_graph_no_vocab_limit import json_cfg_to_graph_token_based
        #return json_cfg_to_graph_token_based(file_name)
        from graph_dataflow import dataflow_graph_token_based
        graph_rep, stats = dataflow_graph_token_based(file_name)
        return graph_rep
