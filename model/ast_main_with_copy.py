...
from ast_dataset_no_vocab_limit import collate_fn
from ast_dataset_no_vocab_limit import ASTDataset
from generic_graph_construction_no_vocab_limit import GenericGraphConstruction
...
class NMT:
    ...
    def train_epoch(self, epoch, split="train"):
        ...
        for step, data in enumerate(dataloader):
            self.logger.info("loading:{}, {}".format(data["graph_data"], data["tgt_seq"]))
    ...
    def evaluate(self, epoch, split="val"):
        ...
        #accuracy = exact_match / len(pred_collect)
    ...
    def translate_top_k(self):
        ...
        for data in dataloader:
            ...
            pred = self.model.translate(
                batch_graph=batch_graph, oov_dict=oov_dict, beam_size=beam_size, topk=top_k
            )

            pred_ids = pred[:, 0, :]  # we just use the top-1

            pred_str = wordid2str(pred_ids.detach().cpu(), ref_dict)
            print("test: gt_str={} -> pred_str={}".format(gt_str, pred_str))

            pred_collect.extend(pred_str)
            gt_collect.extend(gt_str)

            pred_strs = []
            pred_collect_top_1.extend(pred_str)
            for i in range(top_k):
                pred_ids = pred[:, i, :]
                pred_str = wordid2str(pred_ids.detach().cpu(), ref_dict)
                pred_strs.extend(pred_str)
                pred_collect_top_k.extend(pred_str)

        score = self.metrics["BLEU"].calculate_scores(ground_truth=gt_collect, predict=pred_collect_top_1)
        self.logger.info(
            "Evaluation results in `{}` split: BLEU-1:{:.4f}\tBLEU-2:{:.4f}\tBLEU-3:{:.4f}\t"
        ...
        with open('inference.csv', mode='w') as inference_file:
            ...
            for g in gt_collect:
                for position in range(top_k):
                    p = pred_collect_top_k[start + position]
                ...
                start += top_k
            print("Accuracy: {}".format(100 * exact_match / len(gt_collect)))
            print("Position: {}".format(sum(positions)/len(positions)) if len(positions) != 0 else -1)
        return score
    ...
    def translate(self):
        ...
        for data in dataloader:
            ...
            file_name = batch_graph.node_attributes[0]["file_name"]
            inference_file_names.append(file_name)
            print("gt_str:{}||||pred_str:{}".format(gt_str, pred_str))
        ...
        with open('inference.csv', mode='w') as inference_file:
            inference_writer = csv.writer(inference_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            count = 0
            for (g, p, f_name) in zip(gt_collect, pred_collect, inference_file_names):
                #inference_writer.writerow([g, p])
                inference_writer.writerow([g, p, f_name])
                p = p.replace("$ string $", "$string$")
                p = p.replace("$ number $", "$number$")
                if g == p:
                    count = count + 1
...
if __name__ == "__main__":
    ...
    runner.logger.info("Train finish, elapsed_time: {}".format(elapsed_time))

    runner.model = Graph2Seq.load_checkpoint(os.path.join(opt["checkpoint_save_path"], opt["name"]), "best.pth").to(runner.device)
    runner.translate()
