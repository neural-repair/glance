...
def make_graph_edits(file_tuple):
    ...
    ast_bug_dot = shift_node_ast_to_dot(ast_bug)
    buggy_dot = os.path.join(cmd_args.save_dir_dot_file, '%s_buggy.dot' % sample_name)
    with open(buggy_dot, 'w') as f:
        f.write(ast_bug_dot)

    ast_fixed_dot = shift_node_ast_to_dot(ast_fixed)
    fixed_dot = os.path.join(cmd_args.save_dir_dot_file, '%s_fixed.dot' % sample_name)
    with open(fixed_dot, 'w') as f:
        f.write(ast_fixed_dot)
    ...
    #dont compute buggy_refs for the time being
