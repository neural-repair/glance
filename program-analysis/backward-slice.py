...
def construct_statements_and_metadata(instance, line_to_col_map, analyzed_lines):
    statements = []
    for line_num in analyzed_lines:
      statement = ''
      for lexeme in instance.file.lexer():
        if lexeme.line_begin() == line_num:
          if lexeme.token() not in ['Newline'] and lexeme.token() != "Comment":
    ...
    for lexeme in instance.file.lexer():
      if lexeme.ent() and lexeme.ent().kindname() == 'Variable':
        data_flow_info[lexeme.ent().name()] = instance.get_refs(lexeme.ent())

    for key, value in data_flow_info.items():
      new_vals = []
      for v in value:
        if v['line'] in analyzed_lines: 
          new_vals.append(v)
      instance.data_flow_info[key] = new_vals
    
    final_control_flow = {}

    for key, value in instance.control_flow_info.items():
      if key:
        key = int(key)
        if key not in analyzed_lines:
          continue
        final_control_flow[key] = value
  
    line_num_to_seq = {}

    for seq in statements:
      line_num_to_seq[str(analyzed_lines.pop(0))] = seq.strip()
    
    source_code = {}
    for k in sorted(list(map(int, list(line_num_to_seq)))):
      source_code[k] = line_num_to_seq[str(k)]

    final_data = {
      'data_flow': instance.data_flow_info,
      'control_flow': final_control_flow,
      'source': source_code,
      'buggy_line_num': instance.buggy_line_num,
      'parent_type': instance.parent_type
    }

    return statements, final_data
...
class BackwardSlicing(object):
  def __init__(self, db, file, buggy_line_num) -> None:
      ...
      for lexeme in file.lexer():
        if lexeme.line_begin() == buggy_line_num:
          if (lexeme.ent() and lexeme.ent().kindname() == 'Variable') or (lexeme.token() == 'Identifier' and lexeme.ent() and lexeme.ent().parent() and lexeme.ent().parent().kindname() in ['Function', 'Class', 'File', 'Method']):
            self.parent_type = lexeme.ent().parent().kindname()
            lines = set()
  ...
  def get_refs(self, ent):
    info = []
    for ref in ent.refs():
      info.append({ 'line': ref.line(), 'edge': ref.kind().longname().split('web Javascript')[-1].strip() })
    return info

  def get_line_column_map_and_lines(self, lines):
      line_to_col_map = {} # stores the lines where some parts of the line needs to be included (not the whole line)
      updated_lines = lines.copy() if lines else set()
      updated_lines.add(self.buggy_line_num)
      ...
      return (line_to_col_map, updated_lines)
  ...
  def extract_control_flow(self, entity):
      cf_graph = entity.control_flow_graph()
      if cf_graph:
        for node in cf_graph.nodes():
          if node.end_node():
            if node.kind() in ['while-for', 'if', 'try', 'try-catch']:
              self.control_flow_info[node.line_begin()] = {'type': node.kind()}
              for child_node in node.children():
                if node.child_label(child_node) == 'yes':
                  self.control_flow_info[node.line_begin()]['yes'] = child_node.line_begin() if child_node.line_begin() else node.line_begin() + 1
                elif node.child_label(child_node) == 'no':
                  self.control_flow_info[node.line_begin()]['no'] = child_node.line_begin() if type(child_node) == 'object' else node.end_node().line_begin()
            else:
              self.control_flow_info[node.line_begin()] = {
                'next_line': node.end_node().line_begin(),
                'type': node.kind()
              }
          elif node.line_begin() and node.line_begin() not in self.control_flow_info:
            if node.children():
              for child_node in node.children():
                self.control_flow_info[node.line_begin()] = {
                  'next_line': child_node.line_begin(),
                  'type': node.kind()
                }
            else:
              self.control_flow_info[node.line_begin()] = {
                'next_line': node.line_begin() + 1,
                'type': node.kind()
              }
  ...
  def _record_conditional_scope(self):
      ...
      for lexeme in self.file.lexer():
        if lexeme.token() not in ['Whitespace', 'Newline']:
          ...
          if self.parent_type in ['Function', 'Method']:
  ...
  def run(self):
      ...
      return self.get_line_column_map_and_lines(set([a for a in analyzed_lines if a is not None]))
...
def process_single_file(project_path):
  ...
  [buggy_file_col_map, buggy_file_lines] = bs.run()
  statements, metadata = construct_statements_and_metadata(bs, buggy_file_col_map, buggy_file_lines)
...
def test_backward_slice():
  ...
  with open('./test-slice-js/file_line_map.json') as f:
    ...
    for item in file_line_map:
      ...
      [line_col_map, lines] = bs.run()
      actual_statements, _ = construct_statements_and_metadata(bs, line_col_map, lines)
...
def process_split_projects(project_path, slice_type):
  subdir = project_path.split('/')[-1]
  json_file_name = 'metadata_{}_{}.json'.format(subdir, slice_type)
  json_file_exists = os.path.isfile(json_file_name)
  with open(json_file_name, 'w' if not json_file_exists else 'r+') as json_file:
    json_data = json.load(json_file) if json_file_exists else []
    ...
    def generate_slice(row):
      ...
      if (os.path.exists(folderpath_db_map[udb] + '/' + buggy_file) and os.path.exists(folderpath_db_map[udb] + '/' + fixed_file)):
        ...
        if slice_type in ['single', 'dual']:
          try:
            ...
            if db_file:
              ...
              try:
                [buggy_file_col_map, buggy_file_lines] = buggy_file_slice.run()
                if buggy_file_lines == 'No lines':
                  return None
                _, metadata = construct_statements_and_metadata(buggy_file_slice, buggy_file_col_map, list(set(buggy_file_lines)))
                metadata['target'] = file_obj['fixed_line']
                metadata['data_type'] = 'single'
                metadata['json_file'] = json_file_name
                metadata['file_prefix'] = file_obj['buggy_file'].split('_buggy.js')[0]
                # print('METADATA    ::', json.dumps(metadata))
        ...
        if slice_type == 'union':
          try:
            db_fixed_file = db.lookup(fixed_file, 'File')
            db_buggy_file = db.lookup(buggy_file, 'File')
            if db_fixed_file and db_buggy_file:
              fixed_file_slice = BackwardSlicing(db, db_fixed_file[0], int(buggy_line_num))
              buggy_file_slice = BackwardSlicing(db, db_buggy_file[0], int(buggy_line_num))
              print('Doing backward slice on fixed file {}......'.format(fixed_file))
              try:
                [fixed_file_col_map, fixed_file_lines] = fixed_file_slice.run()
                [buggy_file_col_map, buggy_file_lines] = buggy_file_slice.run()
                if fixed_file_lines == 'No lines' or buggy_file_lines == 'No lines':
                  return None
                _, metadata = construct_statements_and_metadata(buggy_file_slice, buggy_file_col_map, sorted(list(set(buggy_file_lines + fixed_file_lines))))
                metadata['target'] = file_obj['fixed_line']
                metadata['data_type'] = 'union'
                metadata['json_file'] = json_file_name
                metadata['file_prefix'] = file_obj['buggy_file'].split('_buggy.js')[0]
                # print('METADATA    ::', json.dumps(metadata))
...
if __name__ == '__main__':
    ...
    slice_type = sys.argv[2] or 'dual' # choices = ['single', 'dual', 'union']
    ...
    if project_type == 'multiple':
      for dir in project_path:
        ...
        process_split_projects(dir, slice_type)
